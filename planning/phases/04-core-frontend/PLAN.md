...
## Objectives
- [ ] **AuthContext**: Update to handle JWT and user state persistence.
- [ ] **Home Page**: Update to fetch products from backend and enable "Add to Cart".
- [ ] **CartPage**: Create a dedicated page/drawer to manage items and quantities.
- [ ] **Checkout Flow**: 
    - [ ] Login/Register modal for unauthenticated users.
    - [ ] CheckoutModal to confirm delivery details.
    - [ ] Redirect to `wa.me` with a formatted order summary.
    - [ ] Clear cart after successful redirection.
- [ ] **Profile**: Enable users to update their delivery details.
